...
<div class="container-fluid page-category d-none d-sm-block min-w1270">
	<div class="row">
				    ...
				    <div class="panel panel-default">
					    ...
					    <div class="panel-body">
						    ...
						    @if($cateList)
							    ...
							    <div class="list-group-item"><a href="{{action('ProductController@productCate',['slug'=>$item->slug,'cate_id'=>$item->id])}}@if($q)?q={{str_replace(' ','+', $q)}}@endif">{{$item->name}}</a></div>
							    @endforeach
						    @endif
						  </div>
					    </div>
		...
		<section class="col-xs-12 col-sm-12 col-md-10 col-lg-10 col-xl-10">
			<div class="category-wrap">
				...
				<h3> {{$category->name}}:{{count('$products')}} kết quả </h3>
				</div>
				@if($products)
				<div class="inner">
				...
				<div class="pagin-wrap">
					<ul class="pagination">
						...
						<li><a href="{{action('ProductController@productCate',['slug'=>$category->slug,'cate_id'=>$category->id])}}@if($q)?q={{str_replace(' ','+', $q)}}@endif&page={{$products->currentpage() -1 }}">&laquo;</a></li>
						@endif
						@for ($i =1; $i<= $products->lastpage(); $i= $i +1)
						<li class="{!!($products->currentpage()==$i) ? 'active':''!!}">
						<a href="{{action('ProductController@productCate',['slug'=>$category->slug,'cate_id'=>$category->id])}}@if($q)?q={{str_replace(' ','+', $q)}}@endif&page={{$i}}">{!!$i!!}</a></li>
						@endfor
						@if($products->currentpage() !=$products->lastpage())
						<li><a href="{{action('ProductController@productCate',['slug'=>$category->slug,'cate_id'=>$category->id])}}@if($q)?q={{str_replace(' ','+', $q)}}@endif&page={{$products->currentpage() + 1 }}">&raquo;</a></li>
						@endif
					</ul>
				</div>
			</div>						
	...
	<div class="mb-container page-mb-master">
		<div class="header-box">
			<h3 class="header-title">
				{{$category->name}}:{{count('$products')}} kết quả
			</h3>
		</div>
		<div class="inner">
			@if($products)
